...
# file_macros, file_labels, file_macros_description, file_includes, file_variables_type, file_structure_variables_types
my (%pliki_opis, %pliki_zmienne, %pliki_funkcje, %pliki_zmienne_opis, %pliki_funkcje_opis,
    ...
    %pliki_label, %pliki_label_opis, %pliki_label_value,
...
foreach my $p (@pliki)
{
    ...
    my ($aktualna_zmienna, $aktualna_zmienna_opis, $aktualna_zmienna_wartosc, $funkcja, $typ,
        $struktura, $inside_struc, $struc_name, $makro, $label, $aktualny_typ);
    $typ = 0;
    $funkcja = 0;
    $struktura = 0;
    $inside_struc = 0;
    ...
    if ( ! /^\s*$/ )
    {
        ...
        # labels
        elsif ( /^\s*((\%i?)?|\.)label\s+(\w+)\s+=\s+([a-z\$0-9]+)$/i )
        {
            $pliki_zmienne{$key}[++$#{$pliki_zmienne{$key}}] = $3;
            $pliki_zmienne_opis{$key}{$3} = $pliki_opis{$key};
            $pliki_zmienne_wartosci{$key}{$3} = $4;
        }
    ...
    while ( <$asm> )
    {
        ...
        {
            $pliki_label{$key}[++$#{$pliki_label{$key}}] = $aktualna_zmienna;
            $pliki_label_opis{$key}{$aktualna_zmienna} = $aktualna_zmienna_opis;
            $label = 0;
        }
        elsif ( $makro )
        {
        ...
        foreach (@{$pliki_zmienne{$key}})
        {
                ...
                $pliki_zmienne_wartosci{$key}{$_} ne "" )
            {
                ...
                print $dox "/**\n"
                    ...
                    ."\n */\n"
                    ."label $_ = $pliki_zmienne_wartosci{$key}{$_}; \n"
                    ;
            }
            else
            {
...
#           }
    ...
    if ( defined($pliki_label{$key}) && (@{$pliki_label{$key}} > 0) )
    {
        foreach (@{$pliki_label{$key}})
        {
            # my $label_proto = "";
            print $dox "/**\n"
                .$pliki_label_opis{$key}{$_}
                ."\n */\n"
                ."label $_ = $pliki_label_value{$key}{$_};\n";
        }
    }
